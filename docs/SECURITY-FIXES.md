...
A revisão preservou o design original do repositório e reorganizou o backend em torno de PostgreSQL, sessões protegidas e autorização por titular. A tabela resume as mudanças verificadas no código entregue.
...
| Sessão | O cliente dependia de múltiplas áreas e scripts inline. | `/api/me` restaura sessão, uma única área do cliente atualiza o estado e o logout limpa a sessão. | `legacy-public.js`, `/api/me` |
...
| Recuperação de 2FA | Não havia estado por conta para recuperação controlada. | A desativação/reativação só ocorre para outra conta administrativa, por sessão administrativa autorizada, com motivo obrigatório, CSRF e auditoria em `admin_events`; auto-desativação e fluxo pelo cliente são recusados. | `PATCH /api/admin/users/<public_id>/2fa`, `admin_2fa_disabled_at`, `admin.js` |
...
| Colisão de CSS | Classes genéricas `.card`, `.nav` e `.top` eram compartilhadas por público, cliente e admin. | O design original é preservado em arquivos próprios `legacy-inline.css`/`legacy-3d.css`; admin continua isolado em `admin.css`. | `static/css/legacy-*.css`, `static/css/admin.css` |
| Acessibilidade de modal | O fluxo usava modais com handlers inline e foco não controlado. | O markup original foi preservado, handlers foram externalizados em `data-legacy-action` e o script controla Escape/fluxo autenticado. | `index.html`, `legacy-public.js` |
